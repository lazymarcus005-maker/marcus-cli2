...
describe("Jev harness shadow coordinator",()=>{
  it("observes failure/progress/review without changing deterministic harness state",async()=>{
    ...
    coordinator.observeToolResult({event:{toolName:"run_test",input:{command:"npm test"},details:{status:"failed",exitCode:1}},harness,runId:"r",changedFiles:["a.ts"],diagnostic:"failed"});
    harness.onFailure("same failure","v1",false);
    coordinator.observeToolResult({event:{toolName:"run_test",input:{command:"npm test"},details:{status:"failed",exitCode:1}},harness,runId:"r",changedFiles:["a.ts"],diagnostic:"failed"});
    coordinator.observeToolResult({event:{toolName:"review",details:{Changed:{},Tested:[],TaskEvidence:[],RemainingRisk:["risk"],UnresolvedIssue:[]}},harness,runId:"r",changedFiles:[],diagnostic:"review"});
