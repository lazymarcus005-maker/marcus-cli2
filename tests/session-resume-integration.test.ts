...
describe("session resume",()=>{
  it("continues the Pi transcript stored under the durable session directory",async()=>{
    const root=await mkdtemp(path.join(os.tmpdir(),"macus-resume-")); const requests:any[]=[];
    ...
    const a=new PiAgentKernel(root,cfg,sid,store,"primary"); await a.create(); await a.prompt("first-message"); await a.current.waitForIdle(); await a.dispose();
    const b=new PiAgentKernel(root,cfg,sid,store,"primary"); await b.create(); await b.prompt("second-message"); await b.current.waitForIdle();
    expect(requests.length).toBeGreaterThanOrEqual(2);
    const latest=JSON.stringify(requests.at(-1).messages);
    expect(latest).toContain("first-message"); expect(latest).toContain("second-message");
    await b.dispose();store.close();delete process.env.MACUS_TEST_KEY;await new Promise<void>(r=>server.close(()=>r()));await rm(root,{recursive:true,force:true});
  },15000);
});
