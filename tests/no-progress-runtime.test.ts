...
function commandCall(id:string){
  return [
    {id:"c"+id,object:"chat.completion.chunk",created:1,model:"mock",choices:[{index:0,delta:{role:"assistant",tool_calls:[{index:0,id,type:"function",function:{name:"run_command",arguments:JSON.stringify({command:"exit 7"})}}]},finish_reason:null}]},
    {id:"c"+id,object:"chat.completion.chunk",created:1,model:"mock",choices:[{index:0,delta:{},finish_reason:"tool_calls"}]}
  ];
}

describe("runtime no-progress enforcement",()=>{
  it("blocks repeated identical failing shell attempts without sending another provider request",async()=>{
    const root=await mkdtemp(path.join(os.tmpdir(),"macus-no-progress-"));
    let requests=0;
    const server=http.createServer(async(req,res)=>{
      for await(const _ of req){};requests++;
      sse(res,commandCall("cmd"+requests));
    });
    await new Promise<void>(r=>server.listen(0,"127.0.0.1",r));
    const port=(server.address() as any).port;process.env.MACUS_TEST_KEY="k";
    const cfg=structuredClone(DEFAULT_CONFIG);cfg.run.max_no_progress_attempts=3;cfg.run.max_model_turns=20;cfg.features.auto_compaction=false;
    ...
    const kernel=new PiAgentKernel(root,cfg,sid,store,"primary");await kernel.create();kernel.authorize("shell");

    await expect(kernel.prompt("keep retrying the failing command")).rejects.toThrow(/run limit/i);
    expect(requests).toBe(3);
    expect(kernel.runState?.noProgress).toBe(3);
